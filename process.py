...
def run(a,id_thread,name_table):
    ...
    conn = mysql.connector.connect(user = 'root',password = '',host = 'localhost', database = 'carttest')
    mycursor=conn.cursor()
    query = """SELECT id_src from table_migration where id_client = %d and name_table = '%s'""" %(id,name_table)
    mycursor.execute(query)
    if mycursor:
        result = mycursor.fetchall()
        last_id = result[0][0]
...
cnx = mysql.connector.connect(user='root',password = '', database='carttest')
cursor = cnx.cursor()
sql = """update resume set id_process = %d where id = %d""" %(processid,id)
try:
    cursor.execute(sql)
    cnx.commit()
except:
...
conn = mysql.connector.connect(user = 'root',password = '',host = 'localhost', database = 'carttest')
mycursor=conn.cursor()
query = """SELECT status from resume where id = %d""" %(id)
mycursor.execute(query)
status=0
if mycursor:
    result = mycursor.fetchall()
...
if status == 1:
    cnx = mysql.connector.connect(user='root',password = '', database='carttest')
    cursor = cnx.cursor()
    sql = """update resume set status = 3 where id = %d""" %(id)
    try:
        cursor.execute(sql)
        cnx.commit()
    except:
